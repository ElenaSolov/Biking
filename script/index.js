...
const highRoadSliderWindow = page.querySelector('.high-road__gallery');
...
window.onload = function () {
    getHighRoadSliderWindow();
}

highRoadNextBtn.addEventListener('click', showNextSection);
highRoadPrevBtn.addEventListener('click', showPrevSection);

function getHighRoadSliderWindow() {
    highRoadSliderWindow.style.maxHeight = highRoadImgEls[0].height - 1 + 'px';
}
...
let index = 0;
// highRoadSliderWindow.style.maxHeight = "190px"
sliderBtns.forEach(btn => btn.addEventListener('click', changePicture));

function checkBikesCards() {
